...
class San_Kloud {
	...
	*/
	function san_kloud_custom_header_setup() {
		...
		$args = array(
			'default-image'          => '',
			'default-text-color'     => $san_kloud_header_color,
			'width'                  => 940,
			'height'                 => 250,
			'flex-height'            => true,
			'wp-head-callback'       => array( $this, 'san_kloud_header_style' ),
			'admin-head-callback'    => array( $this, 'san_kloud_admin_header_style' ),
			'admin-preview-callback' => array( $this, 'san_kloud_admin_header_image' ),
		);

		$args = apply_filters( 'san_kloud_custom_header_args', $args );
		...
		if ( function_exists( 'wp_get_theme' ) ) {
			...
			add_custom_image_header( array( $this, $args['wp-head-callback'] ), array( $this, $args['admin-head-callback'] ), array( $this, $args['admin-preview-callback'] ) );
		}
